...
-- SELECT name as "meetingName", questions.meeting_id as "meetingId", COUNT(questions.meeting_id) FROM  questions INNER JOIN meetings ON questions.meeting_id = meetings.id  WHERE questions.meeting_id IN (SELECT id from meetings WHERE owner_id = 3 ORDER BY id DESC LIMIT 15) GROUP BY (questions.meeting_id, meetings.name); 
-- SELECT name as "meetingName", questions.meeting_id as "meetingId" FROM  questions INNER JOIN meetings ON questions.meeting_id = meetings.id  WHERE questions.meeting_id IN (SELECT id from meetings WHERE owner_id = 3 ORDER BY id DESC LIMIT 15) GROUP BY (questions.meeting_id, meetings.name); 
SELECT name as "meetingName", questions.meeting_id as "meetingId" FROM  questions INNER JOIN meetings ON questions.meeting_id = meetings.id  WHERE questions.meeting_id IN (SELECT id from meetings WHERE owner_id = 3 ORDER BY id DESC LIMIT 15) GROUP BY (questions.meeting_id, meetings.name); 