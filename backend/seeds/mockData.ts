...
function createFakeMeeting(){
    return{
        ...
        date_time: Math.floor(Math.random() * 2)===0?faker.date.recent():faker.date.past()
    }
}
...
function createViews(){
    return{
        ...
        created_at: faker.date.between(new Date(2020, 5, 20, 10, 0, 0),new Date(2020, 5, 20, 11, 0, 0))
    }
}
